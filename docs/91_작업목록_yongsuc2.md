...
| Y-68 | 플레이어 캐릭터 스프라이트 애니메이션 | 중 | 완료 | player.png 스프라이트 시트(4×3, 걷기+공격) 런타임 슬라이싱, CharacterView 프레임 애니메이션(걷기/공격 모션 전환), BattleView 플레이어에 자동 적용 |
...
| Y-71 | 적 분노 공격 스킬 시스템화 | 중 | 완료 | enemy_bunno_attack 스킬 추가, Battle.ts 하드와이어 분노 공격 제거, battle.data.json rage 섹션 삭제, ragePerAttack enemy.data.json 연동 |
